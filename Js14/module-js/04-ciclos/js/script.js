
// const nombres = [
//     { nombre: 'freddy',}, 
//     { nombre: 'michael'} , 
//     { nombre: 'rafa'}
// ];

// const person = {
//     nombre: 'freddy',
//     apellido: 'Krueger',
//     edad: 40,
// };

// console.log(nombres[1])
// console.log(nombres[0])

// for (let i = 0; i < nombres.length; i++) {
//     console.log(nombres[i].nombre)
//     // const element = array[index];
// }

/*let vuelta = 0;

//while(vuelta < 10) {
 //   console.log(vuelta, 'vuelta')
 //   if(vuelta % 2 === 0) {
 //       console.log(`${vuelta} es par`)
  //  } else {
 //       console.log(`${vuelta} es impar`)
    }
    // console.log('se cumpli condicion con while')
 //   vuelta++
}

// for ( killer in person ) {
//     console.log(killer)
//     console.log( person[killer] )
// }



/*
    Ejercicio 1:
    Pedir al usuario usuario un numero entre 1 y 10
    Imprimir la tabla de multiplicar de ese número
    p.ej.
    3 x 1 = 3
    3 x 2 = 6
*/
// const number = prompt('Ingrese un numero entre 1 y 10');
// if (number > 0 && number < 100) {
//     for (let i = 0; i < 11; i++) {
//         let element = (number * i);
//         console.log(`${number} x ${i} = ${element}`)
//     }
// }


/*
Ejercicio 2:
Pedir al usuario un numero entre 10 y 100
Imprimir todos los numeros pares que existen entre 1 y ese numero
*/

// const number = prompt('Ingrese un numero entre 10 y 100');
// if (number > 10 && number < 100) {
//     for (let i = 0; i < number + 1 ; i++) {
//         if (number % 2 === 0){
           
//             console.log (i)
            
//         }
        
//     }
// }


// /*
// Ejercicio 3:
// Imprimir en consola la suma total de todos los digitos de una cantidad p.ej. "1234" -> 10
// */

// const number = prompt('Ingrese digitos para sumarlos')
// number = []


/*
Ejercicio 4:
Imprimir en consola el siguiente patrón

*
**
***
****
*****
******
*******
********
*********
**********

*/

// solicitar un string al usuario
// almacenar el string
// crear la funcion 
// aplicar metodo correspondiente al string
// guardar la salida del metodo
// imprimir la salida 


// separar el string en un arreglo
// separar los subindices del arreglo en pares e impares
// aplicar metodos


let frase = prompt('Ingrese una frase a convertir');

