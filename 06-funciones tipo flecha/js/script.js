//ejercicio de invertir un string

let palabra = prompt('ingrese una palabra')
let cambio = palabra.split("")
let cambio2 = ((cambio.reverse()).join("")).toString()
...
const cartShoppings = [
  ...
  const shoppingCartFunction = (cartShoppings, productType) => {
    const coincidencias = [];
    cartShoppings.forEach((product) => {
        if(product.type === productType) {
            coincidencias.push(product)
        }
    });
...
shoppingCartFunction(cartShoppings, "chip")
shoppingCartFunction(cartShoppings, "drink")


const searchProductpriceLess50 = (cartShoppings, productPrice) => {
    const coincidencias = [];
    cartShoppings.forEach((product) => {
  ...
  searchProductpriceLess50(cartShoppings, 50)

  const searchProductpriceGreater50 = (cartShoppings, productPrice) => {
    const coincidencias = [];
    cartShoppings.forEach((product) => {
        if(product.price >= productPrice) {
            coincidencias.push(product)
  ...
  searchProductpriceGreater50(cartShoppings, 50)
...
totalPrice(cartShoppings)


function totalPrice(data,type = null){
    let suma = 0
    data.forEach((product)=>{
...
totalPrice(cartShoppings)
totalPrice(cartShoppings,"chip")
totalPrice(cartShoppings,"drink")
